...
mapz = np.zeros(step)
mapk = np.zeros(step)
for kk in range(2, step):
    N = 50*kk     # 产生1000个100个链路的样本 （region = 500）
    MAPS = 1000   # 地图数
    ...
    # ============================ #
    Taround1 = np.zeros((MAPS, N, M1, M1))
    Taround2 = np.zeros((MAPS, N, M2, M2))
    Taround3 = np.zeros((MAPS, N, M3, M3))
    Raround1 = np.zeros((MAPS, N, M1, M1))
    Raround2 = np.zeros((MAPS, N, M2, M2))
    Raround3 = np.zeros((MAPS, N, M3, M3))
    tden = np.zeros((MAPS, N, 3))
    rden = np.zeros((MAPS, N, 3))
    Hall = np.zeros((MAPS, N, N))
    Sumrate = 0
    SumrateAA = 0
    Xall = np.zeros((MAPS, N, 1))
    lengthall = np.zeros((MAPS, N, 1))
    # =======main func===========
    for k in range(0, MAPS):
        ...
        tden, rden, Taround1, Taround2, Taround3, Raround1, Raround2, Raround3, Xall, lengthall, Hall = storagedataset(
            ...
            len, Taround1, Taround2, Taround3, Raround1, Raround2, Raround3, Xall, lengthall, Hall)  # Xall存储FP迭代后的X
        print(k)
    # ==================存储到文件================== #
    Taround1.tofile('1000Tard1.dat', sep=',', format='%.64f')
    Taround2.tofile('1000Tard2.dat', sep=',', format='%.64f')
    Taround3.tofile('1000Tard3.dat', sep=',', format='%.64f')
    Raround1.tofile('1000Rard1.dat', sep=',', format='%.64f')
    Raround2.tofile('1000Rard2.dat', sep=',', format='%.64f')
    Raround3.tofile('1000Rard3.dat', sep=',', format='%.64f')
    lengthall.tofile('1000lenall.dat', sep=',', format='%.64f')
    tden.tofile('1000tden.dat', sep=',', format='%.64f')
    rden.tofile('1000rden.dat', sep=',', format='%.64f')
    Hall.tofile('1000Hall.dat', sep=',', format='%.64f')
    Xall.tofile('1000Xall.dat', sep=',', format='%.64f')
    # =============================================== #
    mapx[kk] = N
    ...
    print('Aver-sumrate:', np.sum(ratess)/MAPS, 'Mbps')
    print('All-Active-Aver-sumrate:', np.sum(ratess1)/MAPS, 'Mbps')
"""
    # ==================LOAD & Compare=========================== #
    Taround1t = np.fromfile('50Tard1.dat', dtype=np.float64, sep=',')
    Taround2t = np.fromfile('50Tard2.dat', dtype=np.float64, sep=',')
    Taround3t = np.fromfile('50Tard3.dat', dtype=np.float64, sep=',')
    Raround1t = np.fromfile('50Rard1.dat', dtype=np.float64, sep=',')
    Raround2t = np.fromfile('50Rard2.dat', dtype=np.float64, sep=',')
    Raround3t = np.fromfile('50Rard3.dat', dtype=np.float64, sep=',')
    lengthallt = np.fromfile('50lenall.dat', dtype=np.float64, sep=',')
    Hallt = np.fromfile('50Hall.dat', dtype=np.float64, sep=',')
    Xallt = np.fromfile('50Xall.dat', dtype=np.float64, sep=',')
    tdent = np.fromfile('50tden.dat', dtype=np.float64, sep=',')
    rdent = np.fromfile('50rden.dat', dtype=np.float64, sep=',')
    Taround1t = np.reshape(Taround1t, (MAPS, N, 5, 5))
    Taround2t = np.reshape(Taround2t, (MAPS, N, 15, 15))
    Taround3t = np.reshape(Taround3t, (MAPS, N, 31, 31))
    Raround1t = np.reshape(Raround1t, (MAPS, N, 5, 5))
    Raround2t = np.reshape(Raround2t, (MAPS, N, 15, 15))
    Raround3t = np.reshape(Raround3t, (MAPS, N, 31, 31))
    lengthallt = np.reshape(lengthallt, (MAPS, N, 1))
    Hallt = np.reshape(Hallt, (MAPS, N, N))
    Xallt = np.reshape(Xallt, (MAPS, N, 1))
    tdent = np.reshape(tdent, (MAPS, N, 3))
    rdent = np.reshape(rdent, (MAPS, N, 3))
    print((Taround1 == Taround1).all())
    print(Hallt.shape, '-', Hall.shape)
    print((Hall == Hallt).all())
    print('!!!???', (tdent == tden).all())
    print(Hall[2, 10:15, 10:15])
    print(Hallt[2, 10:15, 10:15])
"""
"""    
print(mapx)
print(mapy)
